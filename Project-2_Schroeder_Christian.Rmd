...
Because there are minimal parameters available, tune mtry, maybe tune ntree

```{r}
set.seed(useed)

mtryGrid <- data.frame(k=seq(1,5))
...
data.rdf.i <- train(ClassTarp~Red+Green+Blue+Green:Blue, data=data,
                    tuneGrid = mtryGrid,
                    method="rf",
                    metric="Accuracy",
                    trControl=control)
...
data.rdf.i$results %>% slice_max(Accuracy)
```
...
try the different types of SVM
...
### Conclusion \#1 Would adding the lat/lon coordinates have helped the accuracy?
